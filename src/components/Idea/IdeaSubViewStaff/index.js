...
function IdeaSubViewStaff({ ideaData, subData,  paginationIdea}) {
    ...
    const loadDataIdea = async () => {
        ...
        await AuthRequest.get(API_PATHS.SHARED.IDEA + "/list", {
            params: {
    ...
    const renderCardHeader = (item) => {
        return (
            <CardHeader
                ...
                action={renderActionIdea(item.id, item?.createBy)}
        ...
        return (
            <CardContent>
                <div style={{ display: "flex" }}>
                    ...
                    <Tooltip title={"Detail Idea"}>
                        <label
                            onClick={()=>{navigate(`/idea/${item.id || "NDM3YzBiMzktMDBlNy00ZDk3LTgzMTctOTE3NzIwYzJkMzlh"}`)}}
                            style={{
    ...
    const renderFooter = () => {
        return (
            <div style={{ marginTop: 15, float: "right" }}>
                <Pagination
                    count={pagination?.total || paginationIdea?.total || 10}
                    onChange={(value) =>
...
export default IdeaSubViewStaff;
