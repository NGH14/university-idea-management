...
const toastMessages = {
    ...
    ERR_USER_NOT_FOUND: "User not found !!",WAIT: "Please wait...",
...
function IdeaDetailView(){
            ...
            <CardContent>
                <div style={{ display: "flex" }}>
                    ...
                    <Tooltip title={"Detail submission"}>
                        <label
                            onClick={()=>{navigate(`/submission/${data.submissionId || "NDM3YzBiMzktMDBlNy00ZDk3LTgzMTctOTE3NzIwYzJkMzlh"}`)}}
                            style={{
    ...
    const renderTop = () => {
        if(data?.create_by !== state?.dataUser?.email && state?.dataUser?.role !== ROLES.ADMIN && state?.dataUser?.role !== ROLES.MANAGER) {
            return null
        }
        return <div style={{display: "flex", width: "100%", justifyContent: "right", marginBottom: 10}}>

                <Button startIcon={<EditIcon />} variant={"contained"} size={"small"} onClick={()=>onOpenModal()}>
                Update idea
            </Button>
