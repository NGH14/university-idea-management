...
function DetailView(){
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (res?.data?.succeeded) {
                ...
                const newValue = {...value, initial_date_v: moment(value?.initial_date).format("DD/MM/YYYY"), final_date_v: moment(value?.final_date).format("DD/MM/YYYY")}
        ...
        try {
            ...
            if (res?.data?.succeeded) {
                setStatus({
                    ...
                    titleNotification: "Update Submission success",
            ...
            setStatus({
                ...status,
                visibleNotification: true,
                titleNotification: "Update Submission error",
                typeNotification: "error",
            });
        }
    };
    ...
    const renderDetailForm = () => {
        return <fieldset style={{border: "2px solid gray", padding: 12, borderRadius: 8}}>
    ...
    const renderIdeaView = () => {
        return <fieldset style={{border: "2px solid gray", padding: 12, borderRadius: 8, }}>
            <legend style={{fontWeight: 'bold', padding: 8, fontSize: 22, display: "flex"}}>Idea </legend>
            <DetailForm initialValue={initialValue}/>
        </fieldset>
    }
